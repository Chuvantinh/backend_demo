...
class PhqReminderPushFollowup implements IScheduleTaskTimer {
  ...
  public async setup(
    ...
    if (!args.userId) {
      logT.error(
        "Missing Argument in scheduleNotifyPhqReminderFollowup",
        Error
      );
      return;
    }
    ...
    logT.info(
      `Scheduling PUSH_NOTIFY_PHQ_REMINDER_FOLLOWUP for ${scheduleTime.toISOString()}`
    );
  ...
  public async schedule(dbTask: ScheduledTask, db: Prisma): Promise<ITaskOptions> {
    const t: ITaskOptions = {
      ...
      async callback(task, done) {
        phqReminderPushFollowup.execute(dbTask, db);
        done();
      }
    };
    return t;
...
export const phqReminderPushFollowup = new PhqReminderPushFollowup();